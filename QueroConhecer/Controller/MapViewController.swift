...
//  QueroConhecer
//
//  Created by Ian Pablo on 04/09/20.
...
class MapViewController: UIViewController {
    ...
    func showMessage(type: MapMessageType){
        ...
        let message = type == .authorizationWarning ? "Para usar os recursos do App, você precisa permitir a localização" : "Não foi possível encontrar esta rota"
...
extension MapViewController: MKMapViewDelegate{
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        let type = (annotation as! PlaceAnnotation).types
        let identifier = "\(type)"
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        search.start { (response, error) in
            if error == nil{
                ...
                self.mapView.removeAnnotations(self.point)
                self.point.removeAll()
                for item in response.mapItems{
                    let annotation = PlaceAnnotation(coordinate: item.placemark.coordinate, type: .point)
                    annotation.title = item.name
                    annotation.subtitle = item.phoneNumber
                    annotation.address = Place.getFormattedAddress(with: item.placemark)
                    self.point.append(annotation)
                }
